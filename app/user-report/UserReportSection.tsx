    ...
    useEffect(() => {
        ...
        const fetchGroupedLogs = async () => {
            try {
                ...
                // console.log(`${process.env.NEXT_PUBLIC_API_URL}/logbook/user?${query}`)
                const res = await fetchWithAuth(`${process.env.NEXT_PUBLIC_API_URL}/logbook/user?${query}`);
    ...
    return (
        <div className="">
            ...
            {groupedLogs.length === 0 ? (
                ...
                groupedLogs.map((data) => {
                    // calculate per-user total minutes
                    const totalMinutes = data.entries.reduce((sum: any, entry: { durationUnit: string; durationNumber: number; }) => {
                        const minutes =
                    ...
                    const formatted = `${d}d ${h}h ${m}m`;

                    return (
                        <Card key={data.user.username} className="mb-3">

                            {/* Header: Username + Per-User Duration */}
                                ...
                                <Table striped>
                                    ...
                                    <TableBody className="divide-y">
                                        {data.entries.map((entry: any) => (
                                            ...
                                            >
                                                <TableCell>
                                                    {new Date(entry.logDate).toLocaleDateString("en-GB")}
                                                </TableCell>
